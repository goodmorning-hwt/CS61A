...
def double_eights(n):
    ...
    def func(num, flag):
        if num==0:
            return False
        if flag == 1 and num%10 == 8:
            return True
        return func(num//10, num%10==8)
...
def count_palindromes(L):
    ...
    def is_palindromic(x):
        return True if len(x)<=1 else x.lower()[0]==x[-1] and is_palindromic(x[1:-1])

    return len([x for x in L if is_palindromic(x)])
