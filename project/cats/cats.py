...
def about(subject):
    ...
    # def ans(sentence):
    #     for word in subject:
    #         if lower(word) in lower(sentence):
    #             front = sentence[sentence.find(word) - 1]
    #             end = sentence[sentence.find(word) + len(word)]
    #             if not (lower(front).islower() or lower(end).islower()):
    #                 return True
    #     return False
...
def accuracy(typed, source):
    ...
    matches = sum(1 for t, s in zip(typed_words, source_words) if t == s)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    for word in word_list:
        if word == typed_word:
            return word
        elif diff_function(typed_word, word, limit) < min_diff:
            ans = word
            min_diff = diff_function(typed_word, word, limit)
        elif diff_function(typed_word, word, limit) == min_diff and not backup:
            backup = word

    if backup and ans == typed_word:
        return backup
...
def minimum_mewtations(typed, source, limit):
    ...
    if limit == -1 or typed == source:
        ...
        if typed[0] != source[0]:
            ...
            return int(minimum_mewtations(typed[1:], source[1:], limit))
...
def time_per_word(words, timestamps_per_player):
    ...
    for player_time in timestamps_per_player:
        player_time_per_word = []
        for i in range(1, len(player_time)):
            player_time_per_word.append(player_time[i] - player_time[i - 1])
        times.append(player_time_per_word)
    return words, times
    # END PROBLEM 9
